...
static const char *END_IF = "END_IF";
static const char *START_LOOP = "START_LOOP";
static const char *END_LOOP = "END_LOOP";
...
{
    uint64_t current_label_counter = label_counter;
    label_counter++;
    node_t *relation = statement->children[0];
    node_t *then_block = statement->children[1];
    node_t *else_block = statement->n_children == 3 ? statement->children[2] : NULL;

    generate_relation(relation);
    char *relation_op = relation->data;
    if (strcmp(relation_op, "=") == 0)
        EMIT("jne %s_%ld", ELSE, current_label_counter);
    if (strcmp(relation_op, "!=") == 0)
        EMIT("je %s_%ld", ELSE, current_label_counter);
    if (strcmp(relation_op, ">") == 0)
        EMIT("jle %s_%ld", ELSE, current_label_counter);
    if (strcmp(relation_op, "<") == 0)
        EMIT("jge %s_%ld", ELSE, current_label_counter);
    generate_statement(then_block);
    EMIT("jmp %s_%ld", END_IF, current_label_counter);
    LABEL("%s_%ld", ELSE, current_label_counter);
    if (else_block)
        generate_statement(else_block);
    LABEL("%s_%ld", END_IF, current_label_counter);
...
{
    uint64_t local_label = label_counter;
    uint64_t prev_label = current_label;
    current_label = local_label;
    label_counter++;
    node_t *relation = statement->children[0];
    node_t *body = statement->children[1];

    LABEL("%s_%ld", START_LOOP, local_label);
    generate_relation(relation);

    char *relation_op = relation->data;
    if (strcmp(relation_op, "=") == 0)
        EMIT("jne %s_%ld", END_LOOP, local_label);
    if (strcmp(relation_op, "!=") == 0)
        EMIT("je %s_%ld", END_LOOP, local_label);
    if (strcmp(relation_op, ">") == 0)
        EMIT("jle %s_%ld", END_LOOP, local_label);
    if (strcmp(relation_op, "<") == 0)
        EMIT("jge %s_%ld", END_LOOP, local_label);

    generate_statement(body);
    EMIT("jmp %s_%ld", START_LOOP, local_label);
    LABEL("%s_%ld", END_LOOP, local_label);
...
static void generate_break_statement()
{
    ...
    EMIT("jmp %s_%ld", END_LOOP, current_label);
    // TODO (2.3):
